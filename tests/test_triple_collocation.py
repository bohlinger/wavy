...
def test_spatial_variance(test_data):
    ...
    new = ico.populate(path=str(test_data/"insitu/monthly/Sulafjorden/AR_TS_MO_A-Sulafjorden_202307.nc"))
    new.vars = new.vars.dropna(dim="time")
    
    variance = tc.spatial_variance(new.vars, period_meas=60, varalias='Hs', sd='2023-07-02', ed ='2023-07-03', n_max=2, n_min=2, time_unit="min")
...
def test_merge_variance(test_data):
    ...
    df_var_sat = tc.merge_variance(str(test_data/"triple_collocation")+"/", list_filenames, res_max=None, res_factor=1.0)

    assert len(df_var_sat) == 127
    assert df_var_sat['res'].values[0] == 2.
    assert df_var_sat['res'].values[-1] == 128.
...
def test_calculate_r2_spatial_variance(test_data):
    ...
    data_tc = {'in-situ': xr.open_dataset(path_files + "Norne_ico.nc").Hs.values,
            'satellite': xr.open_dataset(path_files + "Norne_sco.nc").Hs.values,
            'model': xr.open_dataset(path_files + "Norne_mco.nc").Hs.values}

    list_filenames = ['df_spat_var_sat_s3a.csv', 'df_spat_var_sat_s3b.csv']
    df_var_sat = tc.merge_variance(path_files, list_filenames, res_max=None, res_factor=1.0)
    list_filenames = ['df_spat_var_mod_s3a.csv', 'df_spat_var_mod_s3b.csv']
    df_var_mod = tc.merge_variance(path_files, list_filenames, res_max=None, res_factor=1.0)

    df_fit, r2, s_z, cal_cst = tc.calculate_r2_spatial_variance(df_var_sat, df_var_mod, data_tc, 'in-situ', 'satellite', 'model', 5)
...
def test_power_spectra(test_data):
    ...
    df_ps = tc.power_spectra(new.vars, period_meas=10, varalias='Hs', time_unit="min", fs=6, nsample=64, sd='2023-07-02', ed ='2023-07-03')
...
def test_merge_spectra(test_data):
    ...
    df_spec_sat = tc.merge_spectra(str(test_data/"triple_collocation") + "/", list_filenames, res_max=None, res_factor=1.0)
...
def test_calculate_r2_spectra(test_data):
    ...
    cal_cst = {'in-situ':1., 'satellite': 0.95, 'model': 0.91}
    path_files = str(test_data/"triple_collocation") + "/"

    list_filenames = ['df_spectra_sat_s3a.csv', 'df_spectra_sat_s3b.csv']
    df_spec_sat = tc.merge_spectra(path_files, list_filenames, res_max=None, res_factor=1.0)
    list_filenames = ['df_spectra_mod_s3a.csv', 'df_spectra_mod_s3b.csv']
    df_spec_mod = tc.merge_spectra(path_files, list_filenames, res_max=None, res_factor=1.0)

    df_r2 = tc.calculate_r2_spectra(df_spec_sat, df_spec_mod, cal_cst, 'satellite', 'model', np.arange(6.3,18.9+1,6.3))
...
def test_bin_tc(test_data):
    ...
    res_bin = tc.bin_tc(data_tc, metric="rmse", 
                        vmin=.5, vmax=4.5, step=1., 
                        ref_filter="satellite", ref_tc="in-situ", 
                        transfo_func=None, cal=True)
