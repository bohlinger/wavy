...
#now = datetime.now()
now = datetime(2019,11,24)
now = datetime(now.year, now.month, now.day, 18)
init_fname = now.strftime("%Y%m%d" + "T18")
outpath = "/lustre/storeA/project/fou/om/LC-WVF/"

cmd = 'module load fimex'
tmp = os.system(cmd)
...
for lt in lt_lst:
    ...
    ustr = ("fimex "
            ...
            + " --output.config=cdmGribWriterConfig.xml"
            + " --output.file=" + outpath
            + "wave_emni_" + fc_fname + "_test_fc_10u.grib2"
            + " --output.type=grib2 --input.config=wind_height.ncml")
    tmp=os.system(ustr)
...
cmd = ("cat " + outpath + "*test*.grib2 > " 
    + outpath + "wave_enmi_" + now.strftime("%Y%m%d" + "18") + "_prod_fc.grib2")
tmp=os.system(cmd)
cmd = ("rm " + outpath + "*test*")
tmp=os.system(cmd)
