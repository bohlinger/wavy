...
def posterior_predictive_nigp(
    ...
    Grad_fmean_train = np.interp(X_train.ravel(), X_s.ravel(), Grad_fmean.ravel())
    ...
    K = (
        kernel(X_train, X_train, l, sigma_f)
        + sigma_y**2 * np.eye(len(X_train))
        + np.diag(
            np.diag(np.dot(Grad_fmean_train, np.atleast_2d(Grad_fmean_train.ravel())))
        )
        * sigma_x**2
    )
    K_s = kernel(X_train, X_s, l, sigma_f)
    K_ss = (
        kernel(X_s, X_s, l, sigma_f)
        + sigma_y**2 * np.eye(len(X_s))
        + np.diag(
            np.diag(np.dot(Grad_fmean_input, np.atleast_2d(Grad_fmean_input.ravel())))
        )
        * sigma_x**2
    )
    ### possible solutions:
...
def nll_fn_nigp(X_train, Y_train, Grad_fmean, naive=False):
    ...
    def nll_naive(theta):
        ...
        K = (
            kernel(X_train, X_train, l=theta[0], sigma_f=theta[1])
            + theta[2] ** 2 * np.eye(len(X_train))
            + np.diag(np.diag(np.dot(Grad_fmean, np.atleast_2d(Grad_fmean.ravel()))))
            * theta[3] ** 2
        )
        F = (
            0.5 * np.log(det(K))
    ...
    def nll_stable(theta):
        ...
        F = (
            ...
            + 0.5
            * Y_train.T.dot(lstsq(L.T, lstsq(L, Y_train, rcond=None)[0], rcond=None)[0])
            + 0.5 * len(X_train) * np.log(2 * np.pi)
        )
        return F.ravel()
