...
def collocate_sat_and_insitu(sco, ico, twin=5): # discuss option for time window
    '''
    ...
    dts_src = [datetime.datetime.utcfromtimestamp(t.tolist()/1e9) for t in sco.vars['time'].values]
    lats_src = sco.vars['lats'].values
    lons_src = sco.vars['lons'].values
    dts_trgt = [datetime.datetime.utcfromtimestamp(t.tolist()/1e9) for t in ico.vars['time'].values]  # ico.vars
    dts_blck = []
    lons_blck = []
    lats_blck = []
    idx_blck = []
    ...
    for i in range(len(dts_trgt)):
        # idx = list of indexes from sat data which falls in the time window given by twin (in minutes)
        # in in-situ datetimes at index i
        idx = find_included_times(dts_src, dts_trgt[i], twin=twin) # should I take 5 or 10 ?? 
        ...
        if len(idx) > 0:
            ...
            # list of list of indices from sat data which were matched mby in situ 
            idx_blck.append(idx)
            # list of indices from in-situ data which matched some sat data
            blcks.append(i)
    
    blck_dict = {
                 # save the obtained matching list of arrays from sat data into the dictionnary
                 'src_datetime': dts_blck,
                 'src_latitude': lats_blck,
                 'src_longitude': lons_blck,
                 'src_idx': idx_blck,
                 # get datetime, latitude and longitude of in-situ data which matched and save it
    ...
    for i in range(len(blck_dict['trgt_datetime'])):
        ...
        # compute distance of every point of sat data matched with the corresponding 
        # in-situ data point i
    ...
    # list_vars_to_filter = ['sea_surface_wave_significant_height', 'latitude', 'longitude', 'time', 'datetime']
    
    # for v in list_vars_to_filter:
    
    #     sco_filter.vars[v] = [sco_filter.vars[v][blck_dict['src_idx'][i]\
    #                             [blck_dict['min_dst_idx'][i]]]\
    #                             for i in range(len(blck_dict['min_dst_idx']))]
    #     ico_filter.vars[v] = [ico_filter.vars[v][idx] for idx in blck_dict['trgt_idx']]
