...
def read_local_ncfiles(**kwargs):
    ...
    # varnames
    varname_list = [
        get_filevarname(v, variable_info, satellite_dict[nID], ncmeta) for v in varalias
    ]
    lonsname = get_filevarname("lons", variable_info, satellite_dict[nID], ncmeta)
    latsname = get_filevarname("lats", variable_info, satellite_dict[nID], ncmeta)
    timename = get_filevarname("time", variable_info, satellite_dict[nID], ncmeta)
    # adjust start and end
    sd = sd - timedelta(minutes=twin)
    ed = ed + timedelta(minutes=twin)
    ...
    varnames = {
        **{varalias[i]: varname_list[i] for i in range(len(varalias))},
        "lons": lonsname,
        "lats": latsname,
        "time": timename,
    }
    var = (
        *tuple(var_sliced[v] for v in varname_list),
        var_sliced[lonsname],
        var_sliced[latsname],
        var_sliced[timename],
    )
...
def read_remote_ncfiles_aggregated(**kwargs):
    ...
    # retrieve sliced data
    ds = read_netcdfs_with_credentials_aggregated(pathlst, remoteHostName, usr, pw)
...
def read_local_20Hz_files(**kwargs):
    ...
    lonstr = get_filevarname("lons", variable_info, satellite_dict[nID], ncmeta)
    latstr = get_filevarname("lats", variable_info, satellite_dict[nID], ncmeta)
    timestr = get_filevarname("time", variable_info, satellite_dict[nID], ncmeta)
