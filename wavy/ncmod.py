...
def read_netcdfs(paths, dim="time", decode_times=None, use_cftime=None, **kwargs):
    logger = logging.getLogger(__name__)
    log_level = str(kwargs.get("logging", "WARNING").upper())
    logger.setLevel(getattr(logging, log_level, logging.WARNING))
    ...
    def process_one_path(path):
        ...
        with xr.open_dataset(
            path, decode_times=decode_times, use_cftime=use_cftime, engine="netcdf4"
        ) as ds:
...
def read_netcdfs_with_credentials_aggregated(
    ...
    ds = xr.open_dataset(
        tmp_path, decode_times=decode_times, use_cftime=use_cftime, engine="netcdf4"
    )
    return ds
...
def read_netcdfs_naive(paths, varnames, varalias, **kwargs):
    ...
    darrays = [process_one_path_netCDF4(p, varnames, varalias) for p in tqdm(paths)]
...
def build_xr_ds_multivar(darray: np.ndarray, varnames: dict, varalias: list):
    import xarray as xr
    ...
    ds = xr.Dataset(
        {
            **{
                varnames[varalias[i]]: xr.DataArray(
                    data=darray[i],
                    dims=[varnames["time"]],
                    coords={varnames["time"]: darray[len_varalias + 2]},
                )
                for i in range(len_varalias)
            },
            varnames["lons"]: xr.DataArray(
                data=darray[len_varalias],
                dims=[varnames["time"]],
                coords={varnames["time"]: darray[len_varalias + 2]},
            ),
            varnames["lats"]: xr.DataArray(
                data=darray[len_varalias + 1],
                dims=[varnames["time"]],
                coords={varnames["time"]: darray[len_varalias + 2]},
            ),
            varnames["time"]: xr.DataArray(
                data=darray[len_varalias + 2],
                dims=[varnames["time"]],
                coords={varnames["time"]: darray[len_varalias + 2]},
            ),
        },
...
def read_netcdfs_KF(paths, dim="time"):
    ...
    datasets = [xr.open_dataset(p, chunks="auto", engine="netcdf4") for p in paths]
    print("Concatenating...")
    ds = xr.concat(
        datasets,
        dim=dim,
...
def read_netcdfs_sel_lru(paths, dlst, varname, dim="time", **kwargs):
    ...
    dataarr = [
        process_one_path_lru(paths[i], dlst[i], varname)
        for i in tqdm(range(len(paths)))
    ]
    logger.info("Concatenate ...")
    combined = xr.concat(
        dataarr, dim, coords="minimal", compat="override", combine_attrs="override"
    )
...
def read_netcdfs_sel(paths, dlst, varname, dim="time", **kwargs):
    ...
    dataarr = [
        process_one_path(paths[i], dlst[i], varname) for i in tqdm(range(len(paths)))
    ]
...
def read_swim_netcdfs(pathlst, varalias):
    ...
    for f in pathlst:
        var, time, lon, lat = read_swim_nc(f, varnamedict)
        varlst.append(var)
        timelst.append(time)
        lonslst.append(lon)
        latslst.append(lat)
    ...
    vardict = {varalias: var, "time": time, "longitude": lons, "latitude": lats}
    return vardict
...
def get_filevarname(varalias, variable_info, srcdict, ncdict, **kwargs):
    ...
    if filevarname is None and varalias in vardefdict.keys():
        ...
        logger.warning(" !!! variable not defined nor " + "available in nc-file !!!")
...
def get_filevarname_list(varalias, variable_info, srcdict, ncdict, **kwargs):
    ...
    if filevarname is None and varalias in vardefdict.keys():
        ...
        logging.warning(" !!! variable not defined nor " + "available in nc-file !!!")
...
def dumptonc_stats(pathtofile, title, time_dt, time_unit, valid_dict):
    ...
    if os.path.isfile(pathtofile):
        ...
        dimsize = None
        # dimensions
        dimtime = nc.createDimension("time", size=dimsize)
        # variables
        nctime = nc.createVariable("time", np.float64, dimensions=("time"))
        ncmop = nc.createVariable("mop", np.float64, dimensions=("time"))
        ncmor = nc.createVariable("mor", np.float64, dimensions=("time"))
        ncrmsd = nc.createVariable("rmsd", np.float64, dimensions=("time"))
        ncmsd = nc.createVariable("msd", np.float64, dimensions=("time"))
        nccorr = nc.createVariable("corr", np.float64, dimensions=("time"))
        ncmad = nc.createVariable("mad", np.float64, dimensions=("time"))
        ncbias = nc.createVariable("bias", np.float64, dimensions=("time"))
        ncSI = nc.createVariable("SI", np.float64, dimensions=("time"))
        ncnov = nc.createVariable("nov", np.float64, dimensions=("time"))
...
def get_varname_for_cf_stdname_in_ncfile(ncdict, stdname):
    lst = [
        ...
        if (
            "standard_name" in ncdict[i].keys()
