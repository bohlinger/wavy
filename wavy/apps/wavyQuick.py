...
def main():
    ...
    parser.add_argument(
        "-sat", metavar="satellite", help="satellite mission, currently available\n \
            \ncmems_L3_NRT:\
            \n s3a - Sentinel-3A\
            \n s3b - Sentinel-3B\
    ...
    parser.add_argument(
        "-l", metavar="satellite list", help="delimited list input for sats", type=str
    )
    parser.add_argument(
        "-sd", metavar="startdate", help="start date of time period to check"
    )
    parser.add_argument(
        "-ed", metavar="enddate", help="end date of time period to check"
    )
    parser.add_argument("-mod", metavar="model", help="chosen wave model")
    parser.add_argument("-var", metavar="varalias", help="alias for chosen variable")
    parser.add_argument(
        "-lt", metavar="lead time", type=int, help="lead time from initialization"
    )
    parser.add_argument(
        "-twin", metavar="time window", type=int, help="time window for collocation"
    )
    parser.add_argument(
        "-dist",
    ...
    parser.add_argument("--show", help="show figure", action="store_const", const=True)
    parser.add_argument("-savep", metavar="savepath", help="save figure to path")
    parser.add_argument("-dump", metavar="outpath", help="dump data to .nc-file")
    ...
    sdate = datetime(
        int(args.sd[0:4]), int(args.sd[4:6]), int(args.sd[6:8]), int(args.sd[8:10])
    )
    ...
    if args.ed is None:
        ...
        edate = datetime(
            int(args.ed[0:4]), int(args.ed[4:6]), int(args.ed[6:8]), int(args.ed[8:10])
        )
        twin = 0
    ...
    if args.mod is None and sa_obj.region not in model_dict:
        ...
        mc_obj = mc(model=args.mod, fc_date=edate, leadtime=args.lt, varalias=args.var)
        # collocation
        coll_obj = coll(
            mc_obj_in=mc_obj, obs_obj_in=sa_obj, distlim=args.dist, leadtime=args.lt
        )
        ...
        # get model collocated values
        mc_obj = mc(
            model=sa_obj.region, fc_date=edate, leadtime=args.lt, varalias=args.var
        )
        results_dict = {
            "valid_date": [edate],
            "date_matches": [
            ...
            "sat_matches": sa_obj.vars[variable_info[args.var]["standard_name"]],
        }
        comp_fig(
            args.mod,
            sa_obj,
