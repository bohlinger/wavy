...
class filter_class:
    ...
    def apply_limits(self, **kwargs):
        ...
        if isinstance(new.varalias, list):
            ...
            if (len(new.varalias) > 1) and (kwargs.get("varalias", None) is None):
                msg = (
                    "Variable {} selected by default. ".format(varalias)
        ...
        llim = kwargs.get("llim", variable_def[varalias]["valid_range"][0])
        ulim = kwargs.get("ulim", variable_def[varalias]["valid_range"][1])
    ...
    def filter_landMask(self, **kwargs):
        ...
        if len_of_indices_tmp == 0:
            ...
            for start_idx, stop_idx in indices:
                logger.info(
                    " start_idx: " + str(start_idx) + "stop_idx: " + str(stop_idx)
                )
                no_chunks += 1
        ...
        print(
            " Number of disregarded values due to land intersections:",
    ...
    def filter_distance_to_coast(self, llim=0, ulim=100000000, **kwargs):
        ...
        # Assign back to class object
        new.vars = ds
        print(" Number of disregarded values:", (len(distance_array) - len(mask)))
        print(" Number of remaining values:", len(new.vars["time"]))
        return new
        ...
        for i, j in indices:
            ...
            for k, l in tgc_indices:
                ...
                if len(tmp_tgc_idx) > kwargs.get("chunk_min", 5):
                    ...
                    logger.warning("Chunk size to small -> not filtered and rejected")
                    pass
        ...
        for i, j in indices:
            ...
            for k, l in tgc_indices:
                ...
                if len(tmp_tgc_idx) > kwargs.get("chunk_min", 5):
                    y = tmp_ds[varalias].values[tmp_tgc_idx]
                    window = kwargs.get("window")
                    ts, _ = runmean(y, window, mode=mode)
                    ts_lst.append(ts)
                    tgc_idx_lst.append(np.array(tmp_idx)[tmp_tgc_idx])
                else:
                    logger.warning("Chunk size to small -> not filtered and rejected")
    ...
    def cleaner_blockStd(y, **kwargs):
        ...
        idx = [i for i in range(len(y)) if (y[i] < uplim and y[i] > lowlim)]
        return idx
    ...
    def despike_blockStd(self, **kwargs):
        ...
        print(
            " Number of disregarded values:", (len(self.vars.time) - len(new.vars.time))
        )
        print(" Number of remaining values:", len(new.vars["time"]))
    ...
    def cleaner_blockQ(y, **kwargs):
        llim_pct = kwargs.get("llim_pct", 0.05)
        ulim_pct = kwargs.get("ulim_pct", 0.95)
    ...
    def slider_chunks(self, **kwargs):
        ...
        for i in range(0, len(new.vars.time), slider):
            ...
            stop_idx = np.min([i + slider + overlap, len(new.vars.time) - 1])
    ...
    def time_gap_chunks(pdtime, **kwargs):
        ...
        mask = (
            (pdtime.diff() > pd.to_timedelta((1.0 / sr) * 2, "seconds"))
            .to_numpy()
            .copy()
        )
    ...
    def filter_footprint_land_interaction(self, **kwargs):
        ...
        print(" Number of disregarded values:", (len(self.vars.lons) - len(ls_idx_lst)))
        print(" Number of remaining values:", len(new.vars["time"]))
        return new
    ...
    def _generate_xtrack_footprints(self, **kwargs):
        ...
        if domain == "cartesian":
            ...
            for i in range(len(lons)):
                ...
                for s in range(n):
                    P_perp_minus, P_perp_plus = (
                        new._generate_xtrack_footprints_in_lonlat(P1, P2, s)
                    )
                    # check if within pulse limited footprint
                    dist = new._distance(P1[0], P1[1], P_perp_minus[0], P_perp_plus[1])
                    if dist > new.vars.fpr.values[i]:
                        pass
                    else:
                        lons_perp = np.transpose(np.array([P_perp_minus, P_perp_plus]))[
                            0
                        ]
                        lats_perp = np.transpose(np.array([P_perp_minus, P_perp_plus]))[
                            1
                        ]
                    # check if footprints intersect with land
                    sea_mask = apply_land_mask(lons_perp, lats_perp, **kwargs)
                    if False in sea_mask:
                        logger.info("Polution by land is detected for index " + str(i))
            ...
            if kwargs.get("rm_bad_neighbours", False) is True:
                ...
                expanded = np.any(
                    np.abs(np.array(ls_idx_lst)[:, None] - invalid) <= k, axis=1
                )
                mask = mask & ~expanded
    ...
    @staticmethod
    def _generate_xtrack_footprints_in_lonlat(P1: tuple, P2: tuple, n=None):
        """
        ...
        coef_dist_latlon = np.cos(np.deg2rad((P1[1] + P2[1]) / 2.0))
    ...
    def _lonlat_to_xy(lon, lat, utmzone):
        P = Proj(proj="utm", zone=utmzone, ellps="WGS84", preserve_units=True)
        return P(lon, lat)
...
def smoother_GP(x, y, X, **kwargs):
    ...
    if "kernel" in kwargs.keys():
        ...
        kernel = WhiteKernel(noise_level=1, noise_level_bounds=(0.5, 1000)) + 1 * RBF(
            length_scale=1, length_scale_bounds=(2, 10.0)
        )
...
def cleaner_GP(x, y, **kwargs):
    ...
    idx = [i for i in range(len(Y)) if (Y[i] < uplim[i] and Y[i] > lowlim[i])]
    return idx
...
def cleaner_linearGAM(x, y, **kwargs):
    ...
    # sample on the input grid
    means = gam.predict(X)
    bounds = gam.prediction_intervals(X, width=kwargs.get("pct", 0.95))
    idx = [i for i in range(len(y)) if (y[i] < bounds[i, 1] and y[i] > bounds[i, 0])]
    return idx
