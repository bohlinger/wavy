...
class insitu_class(qls, fc):
    ...
    def __init__(self, **kwargs):
        ...
        self.name = kwargs.get("name", list(dc.name.keys())[0])
    ...
    def _create_pathlst(self, **kwargs):
        ...
        file_date_incr_unit = self.cfg.wavy_input["file_date_incr_unit"]
        ...
        while tmpdate <= self.ed:
            # get pathtofile
            pathtofile = get_pathtofile(pth_tmplt, strsub, subdict, tmpdate)
            pathlst.append(pathtofile)
            tmpdate = date_dispatcher(tmpdate, file_date_incr_unit, file_date_incr)
        return pathlst
        ...
        if wavy_path is not None:
            ...
            while tmpdate <= date_dispatcher(
                ...
                try:
                    ...
                    path_template = self.cfg.nID["wavy_input"].get("src_tmplt")
                    strsublst = self.cfg.nID["wavy_input"].get("strsub")
    ...
    def _enforce_meteorologic_convention(self, **kwargs):
        ...
        for ncvar in ncvars:
            if (
                "convention" in self.cfg.misc.keys()
                and self.cfg.misc["convention"] == "oceanographic"
            ):
                logger.info("Convert from oceanographic to meteorologic convention")
                self.vars[ncvar] = convert_meteorologic_oceanographic(self.vars[ncvar])
    ...
    def _change_varname_to_aliases(self, **kwargs):
        ...
        for v in self.varalias:
            ncvar = get_filevarname(v, variable_def, vars(self.cfg), self.meta)
        ...
        for c in coords:
            try:  # because only if available
                ncvar = get_filevarname(c, variable_def, vars(self.cfg), self.meta)
    ...
    def populate(self, **kwargs):
        ...
        try:
            ...
            if (
                self._return_extension(self.pathlst[0]) == ".nc"
