...
def _round_to_hour(t):
    ...
    dt = datetime(t.year, t.month, t.day, t.hour, t.minute, t.second, t.microsecond)
    return pd.Timestamp(hour_rounder(dt))


def _draw_colored_track(
    ax, lons, lats, values, cmap, norm, projection, linewidth=2.5, zorder=10
):
    """
...
def _format_threshold_message(stats, n_std):
    ...
    if method == "zscore":
        ...
        rule = (
            f"|modified z-score (median={stats['median_bias']:+.3f}, "
            f"MAD={stats['mad_bias']:.3f})| > {n_std}  "
            f"[≈ outside {stats['lo']:.3f} … {stats['hi']:.3f} m]"
        )
    else:  # iqr
        rule = (
            f"bias outside [Q1−{n_std}×IQR, Q3+{n_std}×IQR] = "
...
def find_outliers(cco, n_std=3, method="zscore", obs_var=None, mod_var=None):
    ...
    if "model_time" in cco.vars:
        df["model_time"] = pd.DatetimeIndex(np.asarray(cco.vars["model_time"])[idx])
...
def plot_outlier_map(
    ...
    if len(ds_track.time) == 0:
        logger.info(
            "[plot_outlier_map] No collocated points in " "±%d min around %s",
            track_window_min,
            outlier_time,
        )
        return None
    ...
    logger.info(
        "[plot_outlier_map] Loading model '%s'%s at %s " "(rounded from %s) …",
        model_nID,
    ...
    gl = ax.gridlines(
        draw_labels=_draw_labels, linewidth=0.4, color="gray", alpha=0.5, linestyle="--"
    )
    if _draw_labels:
        gl.top_labels = False
    ...
    if lc is not None:
        fig.colorbar(lc, ax=ax, label=track_label, fraction=0.033, pad=0.09, shrink=0.8)
...
def plot_all_outliers(
    ...
    if make_distribution:
        sp = (
            os.path.join(save_dir, f"outlier_distribution_{key}.png")
            if save_dir
            else None
        )
    ...
    if make_timeseries:
        sp = (
            os.path.join(save_dir, f"outlier_timeseries_{key}.png")
            if save_dir
            else None
        )
    ...
    for i, t in enumerate(times):
        if not used[i]:
            window_mask = np.array(
                [abs((t2 - t).total_seconds()) <= track_window_min * 60 for t2 in times]
            )
    ...
    logger.info(
        "[plot_all_outliers] %d event(s) detected — " "plotting top %d by max |bias|:",
        len(event_times),
        len(top_times),
    )
    ...
    for rank, t in enumerate(top_times, start=1):
        ...
        sp = (
            os.path.join(save_dir, f"outlier_top{rank:02d}_{key}_{t_str}.png")
            if save_dir is not None
            else None
        )
...
def _square_extent(lons, lats, margin_deg=1.0, min_span_deg=1.0):
    """
    Compute a square (equal lon/lat span) bounding box around a cluster of
    points — used to zoom tightly on the points flagged by a single
    numerical instability pattern (checkerboard, garden-sprinkler, etc.),
    which are usually confined to a small patch of the grid and would
    otherwise render as an illegible dot cluster on a full-domain map.

    The raw lon span is computed via `_lon_extent`'s antimeridian-safe
    unwrap logic (no margin), then both spans are padded out to the larger
    of (lon_span, lat_span, min_span_deg) so the resulting box is square in
    degrees. `min_span_deg` guards against a degenerate box when all
    flagged points collapse onto (near) one location.

    Parameters
    ----------
    lons, lats    : array-like  – coordinates of the flagged points.
    margin_deg    : float       – padding added on each side, in degrees
                                  (default 1.0).
    min_span_deg  : float       – smallest allowed span before padding
                                  (default 1.0), so a single-point or
                                  tightly-clustered pattern still gets a
                                  usable zoom window rather than a
                                  near-zero box.
    ...
    if lons.size == 0:
        raise ValueError("No finite points to compute a zoom extent for.")

    lonmin, lonmax = _lon_extent(lons, margin_deg=0.0)
    latmin = float(lats.min())
    latmax = float(lats.max())

    lon_c = (lonmin + lonmax) / 2.0
    lat_c = (latmin + latmax) / 2.0
    span = max(lonmax - lonmin, latmax - latmin, min_span_deg)
    half = span / 2.0 + margin_deg

    lonmin, lonmax = lon_c - half, lon_c + half
    latmin, latmax = max(lat_c - half, -90.0), min(lat_c + half, 90.0)
...
class outlier_class:
    ...
    def __init__(self, cco, n_std=3, method="zscore", obs_var=None, mod_var=None):
        ...
        self.n_std = n_std
        self.method = method
        self.obs_var = obs_var or ("obs_Hs" if "obs_Hs" in cco.vars else "obs_values")
    ...
    def populate(self, **kwargs):
        ...
        ds_attrs = {k: v for k, v in stats.items() if isinstance(v, (str, int, float))}
    ...
    @classmethod
    def from_dill(cls, path, n_std=3, method="zscore", obs_var=None, mod_var=None):
        """
        ...
        with open(path, "rb") as fh:
            cco = dill.load(fh)
        return cls(cco, n_std=n_std, method=method, obs_var=obs_var, mod_var=mod_var)
    ...
    def plot_overview(
        ...
        if mode == "density":
            ...
            fig.colorbar(
                cf,
                ax=ax,
                label="Number of outliers per cell",
                fraction=0.033,
                pad=0.06,
                shrink=0.85,
            )
        ...
        # ---- title ------------------------------------------------------------
        method_used = (
            self.stats.get("method", self.method) if self.stats else self.method
        )
    ...
    def plot_pattern_zoom(
        ...
        """
        Zoomed, square-extent map focused on the points flagged by one
        detected numerical pattern.

        `plot_overview` shows every outlier over the whole domain; patterns
        like checkerboard noise or garden-sprinkler clusters are usually
        confined to a small patch, so a full-domain view renders them as a
        single unreadable smear. This crops to a square box around just
        that pattern's affected points and re-uses `plot_overview`'s
        scatter/density rendering at that zoom level.

        Parameters
        ----------
        pattern_key     : str  – key into the pattern report, e.g.
                                 "checkerboard", "garden_sprinkler",
                                 "source_term_ringing", "hs_collapse",
                                 "spinup_insufficient".
        pattern_report  : dict, optional – defaults to self.pattern_report
                                 (set by detect_numerical_patterns()).
        margin_deg      : float – padding added around the cluster (def 1.0).
        min_span_deg    : float – minimum box span in degrees, so a very
                                 tight cluster still gets a legible zoom
                                 (default 1.0).
        mode            : "scatter" or "density" – forwarded to plot_overview.
        point_size      : float – marker size for scatter mode.
        projection      : cartopy.crs, optional – default PlateCarree.
        show_full_track : bool – draw the full cco track in grey for context
                                 (default True).
        save_path       : str/Path, optional.
        ...
        -------
        fig : matplotlib.Figure or None
            None if the pattern wasn't detected or has no affected points.
        """
        report = pattern_report if pattern_report is not None else self.pattern_report
        if report is None:
            raise ValueError(
                "No pattern report available; run detect_numerical_patterns() "
                "first or pass pattern_report explicitly."
            )
        if pattern_key not in report:
            raise KeyError(
                f"'{pattern_key}' not in pattern report; available keys: "
                f"{[k for k in report if k != 'summary']}"
            )

        res = report[pattern_key]
        if not res.get("detected") or not res.get("affected_idx"):
            logger.info(
                "[plot_pattern_zoom] Pattern '%s' not detected / no affected "
                "points — nothing to zoom on.",
                pattern_key,
            )
            return None

        idx = np.asarray(res["affected_idx"])
        lons = self.vars["obs_lons"].values[idx]
        lats = self.vars["obs_lats"].values[idx]
        bb = _square_extent(
            lons, lats, margin_deg=margin_deg, min_span_deg=min_span_deg
        )

        # Subset to just this pattern's points, then reuse plot_overview's
        # rendering (scatter/density, land/coast, gridlines, colorbar, ...).
        sub = deepcopy(self)
        sub.vars = self.vars.isel(time=idx)

        fig = sub.plot_overview(
            projection=projection,
            bb=bb,
            margin_deg=0.0,
            point_size=point_size,
            show_full_track=show_full_track,
            mode=mode,
            save_path=None,
        )
        if fig is None:
        ...
        entry_name = (
            PATTERN_REGISTRY[pattern_key]["name"]
            if pattern_key in PATTERN_REGISTRY
            else pattern_key
        )
        ax = fig.axes[0]
        ax.set_title(
            ax.get_title() + f"\nZoomed on: {entry_name}  ({len(idx)} pt(s), "
    ...
    def group_events(self, track_window_min=30):
        ...
        for i, t in enumerate(times):
            if not used[i]:
                dt_sec = np.array([abs((t2 - t).total_seconds()) for t2 in times])
        ...
        **pattern_kwargs         : any additional keyword args forwarded to the
        ...
        if self.cco is not None:
            try:
                ...
                cco_times_s = np.array(
                    pd.DatetimeIndex(self.cco.vars["time"].values).astype("int64")
                    // 10**9
                )
                ...
                impact_lines.append(f"RMSE (all outliers)       = {rmse_all:.3f} m")
                impact_lines.append(
        ...
        for key in keys_to_run:
            ...
            if res["detected"]:
                ...
                if key == "checkerboard":
                    lines.append(f"   parity_fraction  : {res['parity_fraction']:.3f}")
                    lines.append(f"   amplitude_m      : {res['amplitude_m']:.3f} m")
                elif key == "garden_sprinkler":
                    lines.append(f"   flagged windows  : {len(res['windows'])}")
                    ...
                    lines.append(f"   flagged windows  : {res['n_flagged_windows']}")
                    lines.append(
                        f"   mean amplitude   : {res['mean_amplitude_m']:.3f} m"
                    )
                elif key == "hs_collapse":
                    lines.append(f"   collapsed points : {res['n_collapsed']}")
                    lines.append(f"   mean obs_Hs      : {res['mean_obs_hs_m']:.3f} m")
                    ...
                    lines.append(f"   run_start_used       : {res['run_start_used']}")
            else:
    ...
    def suggest_fixes(self, pattern_report: dict | None = None) -> str:
        ...
        if report is None:
            msg = (
                "No pattern report available. " "Run detect_numerical_patterns() first."
            )
            logger.info("%s", msg)
            return msg
        ...
        lines = [
            "=" * 70,
            "  Suggested fixes for detected numerical instabilities",
            "=" * 70,
        ]
    ...
    def plot_event_timeseries(self, event_id, events_df, window_h=3, save_path=None):
        """
    ...
    def plot_track_over_model(
        ...
        _mc_name = (
            model_name if model_name is not None else getattr(self.cco, "name", None)
        )
        ...
        if len(ds_track.time) == 0:
            logger.info(
                "[plot_track_over_model] No collocated points in " "±%d min around %s",
                track_window_min,
                center_time,
            )
            return None
        ...
        # ---- render model layer via quicklook (preserves clean visual style) ---
        if mco is not None:
            try:
                ...
                logger.warning("quicklook failed: %s — using fallback axes.", exc)
                mco = None

        if mco is None:
            # fallback when model loading or rendering fails
            if projection is None:
                projection = ccrs.PlateCarree()
            fig = plt.figure(figsize=(10, 9))
            ax  = fig.add_subplot(1, 1, 1, projection=projection)
        ...
        # ---- satellite track (scatter, coloured by track_color_by) ---------
