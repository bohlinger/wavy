...
def get_AIS_data(bbox, sd=None, ed=None, minspeed=0.5):
    ...
    headers_ships_nsr = {"accept": "test/plain", "Content-Type": "application/json"}
    response_ships_nsr = requests.post(
        url_ships_nsr, json=json_ships_nsr, headers=headers_ships_nsr
    )
    resp_ships_json_nsr = response_ships_nsr.json()
    ships_df = pd.DataFrame(resp_ships_json_nsr["data"])
    ships_df = ships_df[
        ["mmsino", "grosstonnage", "dwt", "length", "breadth", "draught", "shiptypeeng"]
    ]

    # Merge the two dataframes
    df_merge = pd.merge(
        df_request, ships_df, left_on="mmsi", right_on="mmsino", how="left"
    )
    df_merge = df_merge[
        [
            "mmsi",
    ...
    df_merge = df_merge.rename(
        columns={"lon": "lons", "lat": "lats", "datetime": "time"}
    )
