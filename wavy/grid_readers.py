...
def read_ww3_unstructured_to_grid(**kwargs):
    ...
    # get meta data
    varstr = [get_filevarname(v, variable_def, model_dict[nID], meta) for v in varalias]
    lonstr = get_filevarname("lons", variable_def, model_dict[nID], meta)
    latstr = get_filevarname("lats", variable_def, model_dict[nID], meta)
    timestr = get_filevarname("time", variable_def, model_dict[nID], meta)
    ...
    for p in pathlst:
        ...
        for t in times:
            ...
            if dt >= sd and dt <= ed:
                ...
                if len(times) < 2:
                    var = (
                        *tuple(ds[v].data for v in varstr),
                        ds[lonstr].data,
                        ds[latstr].data,
                        t.reshape((1,)),
                    )
                    ...
                    var = (
                        *tuple(ds[v].sel({timestr: t}).data for v in varstr),
                        ds[lonstr].data,
                        ds[latstr].data,
                        t.reshape((1,)),
                    )
...
def get_gridded_dataset(var, t, **kwargs):
    ...
    if kwargs.get("interp") is None:
        ...
        for i in range(len_varstr):
            gridvar, lon_grid, lat_grid = grid_point_cloud_ds(
                var[i], var[len_varstr], var[len_varstr + 1], t, **kwargs
            )
        ...
        for i in range(len_varstr):
            ...
            var_means, lon_grid, lat_grid = grid_point_cloud_interp_ds(
                var[i], var[len_varstr], var[len_varstr + 1], **kwargs
            )
    ...
    ds = xr.Dataset(
        {
            varstr: xr.DataArray(
                ...
                coords={"latitude": lat_grid, "longitude": lon_grid, "time": t},
...
def grid_point_cloud_ds(values, lons, lats, t, **kwargs):
    ...
    gco = gc(lons=lons, lats=lats, values=values, bb=bb, res=res, sdate=t, edate=t)
...
def grid_point_cloud_interp_ds(values, lons, lats, **kwargs):
    ...
    x, y = np.arange(bb[0], bb[1] + res[0] / 2, res[0]), np.arange(
        bb[2], bb[3] + res[1] / 2, res[1]
    )
    ...
    gridvar = griddata(
        points, values, (lon_grid, lat_grid), method=kwargs.get("method", "linear")
    )
