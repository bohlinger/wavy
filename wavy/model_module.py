...
def generate_bestguess_leadtime(model, fc_date, lidx=None, **kwargs):
    ...
    if isinstance(fc_date, list):
        leadtime = [
            generate_bestguess_leadtime(model, date, **kwargs) for date in fc_date
        ]
    else:
        init_times = np.array(model_dict[model]["misc"]["init_times"]).astype("float")
        ...
        if len(gtz) == 0:
            leadtime = int(
                np.abs(np.min(np.abs(diffs)) - model_dict[model]["misc"]["init_step"])
            )
        ...
        if leadtime == 0:
            ...
            logger.warning(
                "Lead time " + str(leadtime) + "h" + " not available for model " + model
            )
...
def read_model_nc_output_lru(filestr, lonsname, latsname, timename):
    ...
    model_time_dt = list(netCDF4.num2date(model_time[:], units=model_time.units))
    f.close()
...
class model_class(qls):
    ...
    def crop_to_period(self, **kwargs):
        ...
        logger.info("Crop to time period: " + str(sd) + " to " + str(ed))
    ...
    def _get_model_filedate(self, fc_date, leadtime, **kwargs):
        ...
        if (
            "init_times" in vars(self.cfg)["misc"].keys()
            and vars(self.cfg)["misc"]["init_times"] is not None
        ):
            init_times = np.array(vars(self.cfg)["misc"]["init_times"]).astype("float")
        else:
            logger.warning("init_times for chosen model not specified in config file")
            logger.warning("Assuming continuous simulation with hourly values")
        ...
        if date_hour in init_times:
            logger.info(
                "Leadtime " + str(leadtime) + " available for date " + str(fc_date)
            )
    ...
    def _make_model_filename(self, fc_date, leadtime, **kwargs):
        ...
        if self.nID in model_dict:
            if "xtra_h" in vars(self.cfg)["misc"]:
                filedate = self._get_model_filedate(fc_date, leadtime, **kwargs)
                pathdate = (
                    filedate
                    + timedelta(hours=leadtime) * vars(self.cfg)["misc"]["lt_switch_p"]
                )
                ...
                for i in range(vars(self.cfg)["misc"]["nr_filedates"]):
                    ...
                    replacestr = (
                        ...
                        + timedelta(
                            hours=leadtime
                            - (leadtime % vars(self.cfg)["misc"]["init_step"])
                        )
                        * vars(self.cfg)["misc"]["lt_switch_f"][i]
                        + timedelta(hours=vars(self.cfg)["misc"]["xtra_h"][i])
                    ).strftime(filedatestr)
                    tmpstr = tmpstr.replace("filedate", replacestr, 1)
                filename = (
                    pathdate.strftime(vars(self.cfg)["wavy_input"]["src_tmplt"])
                    + tmpstr
                )
            else:
                filedate = self._get_model_filedate(fc_date, leadtime, **kwargs)
                if filedate is None:
                    ...
                    if strsublst:
                        subdict = make_subdict(strsublst, class_object_dict=vars(self))
                        path_template = make_pathtofile(
                            path_template, strsublst, subdict, **kwargs
                        )
                    filename = filedate.strftime(path_template) + filedate.strftime(
                        vars(self.cfg)["wavy_input"]["fl_tmplt"]
                    )
    ...
    def _make_model_filename_wrapper(self, fc_date, leadtime, **kwargs):
        ...
        remoteHostName = kwargs.get(
            "remoteHostName", self.cfg.misc.get("remoteHostName")
        )
        ...
        # bound on how many times we are willing to bump the leadtime
        # while looking for an accessible "best guess" file
        ...
        if isinstance(fc_date, datetime) and leadtime != "best":
            ...
            switch = False
            leadtime = generate_bestguess_leadtime(self.nID, fc_date, **kwargs)
            ...
            if leadtime is not None:
                ...
                while switch is False:
                    ...
                    if remoteHostName is not None:
                        filename = build_usr_pw_path(filename, remoteHostName, usr, pw)
                    switch = check_if_ncfile_accessible(filename, **kwargs)
                    if switch is False:
                        logger.warning(
                            "Desired file: " + str(filename) + " not accessible"
                        )
                        logger.warning(
            ...
            filename = [
                self._make_model_filename(date, leadtime, **kwargs) for date in fc_date
            ]
        elif isinstance(fc_date, list) and leadtime == "best":
            leadtime = generate_bestguess_leadtime(self.nID, fc_date, **kwargs)
            filename = [
                self._make_model_filename(fc_date[i], leadtime[i], **kwargs)
    ...
    def _get_files(self, dict_for_sub=None, path=None, wavy_path=None, **kwargs):
        ...
        if wavy_path is not None:
            ...
            while tmpdate <= date_dispatcher(
                self.ed, self.cfg.misc["date_incr_unit"], self.cfg.misc["date_incr"]
            ):
                try:
                    # create local path for each time
                    path_template = vars(self.cfg)["wavy_input"].get("src_tmplt")
                    strsublst = vars(self.cfg)["wavy_input"].get("strsub")
                    subdict = make_subdict(strsublst, class_object_dict=dict_for_sub)
                    path = make_pathtofile(path_template, strsublst, subdict, **kwargs)
    ...
    def list_input_files(self, show=False, **kwargs):
        ...
        if kwargs.get("path") is None and kwargs.get("wavy_path") is None:
            ...
            path = kwargs.pop(
                "path", None
            )  # this causes an error : double path argument LH
            wavy_path = kwargs.pop("wavy_path", None)

            pathlst, _ = self._get_files(
                vars(self), path=path, wavy_path=wavy_path, **kwargs
            )
    ...
    def _enforce_meteorologic_convention(self, **kwargs):
        ...
        for v in self.varalias:
            if (
                "convention" in vars(self.cfg)["misc"].keys()
                and vars(self.cfg)["misc"]["convention"] == "oceanographic"
            ):
    ...
    def _change_varname_to_aliases(self, **kwargs):
        ...
        for v in self.varalias:
            ncvar = get_filevarname(
                v, variable_def, vars(self.cfg), self.meta, **kwargs
            )
        ...
        for c in coords:
            ncvar = get_filevarname(
                c, variable_def, vars(self.cfg), self.meta, **kwargs
            )
    ...
    def _change_stdvarname_to_cfname(self, **kwargs):
        ...
        self.vars["lons"].attrs["standard_name"] = variable_def["lons"].get(
            "standard_name"
        )
        self.vars["lats"].attrs["standard_name"] = variable_def["lats"].get(
            "standard_name"
        )
        self.vars["time"].attrs["standard_name"] = variable_def["time"].get(
            "standard_name"
        )
        # enforce standard_name for variable alias
        for i in range(len(self.varalias)):
            self.vars[self.varalias[i]].attrs["standard_name"] = self.stdvarname[i]
        return self
    ...
    def populate(self, **kwargs):
        ...
        if len(self.pathlst) > 0:
            ...
            ncvar = [
                get_filevarname(v, variable_def, vars(self.cfg), self.meta, **kwargs)
                for v in self.varalias
            ]
            logger.info("")
